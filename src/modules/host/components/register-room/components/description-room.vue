...
const especificaciones = currentPost.especificacionesAmbiente || [];

// Initialize room specification services with existing data
const servicios = ref(
  initializeServices(getServicesByCategory('room'), especificaciones)
);
...
watch(servicios, (newServicios) => {
  ...
  currentPost.especificacionesAmbiente = serviciosActivos.map(servicio => ({
    idAmbiente: 0,
